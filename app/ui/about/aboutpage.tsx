...
export async function AboutPage({params}: { params: { id: string } }) {
    ...
    const {editDate, title, description, author} = await fetchDocumentById<AboutDocument>(id, "history", "about");
    return (
        <>
            ...
            </p>
            {
                session?.user?.email === 'tinbolw@gmail.com' ?
                    <div className="flex justify-center">
                        <Link className={`${buttonTypes.hoverable} text-2xl w-16`}
                              href={`/about/${params.id}/edit`}>Edit</Link>
                    </div> : <></>
            }
        </>
    )
