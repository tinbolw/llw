import { AboutDocument } from "@/app/lib/definitions";
import { fetchDocumentById } from "@/app/lib/api/mongodb/mongo-actions";
import { auth } from "@/auth";
import { buttonTypes } from "@/app/ui/common";
import Link from "next/link";

// todo what is this?
export async function AboutPage({ params }: { params: { id: string } }) {
  const id = params.id;
  const { editDate, title, description, author } =
    await fetchDocumentById<AboutDocument>(id, "history", "about");
  return (
    <>
      <h1 className="text-2xl">{title}</h1>
      <p>{`Last Edited: ${editDate} by ${author}`}</p>
      <br />
      <p className="text-xl">{description}</p>
      {false ? (
        <div className="flex justify-center">
          <Link
            className={`${buttonTypes.hoverable} text-2xl w-16`}
            href={`/about/${params.id}/edit`}
          >
            Edit
          </Link>
        </div>
      ) : (
        <></>
      )}
    </>
  );
}
