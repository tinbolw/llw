 ...
 * @return type
 */
export async function fetchDocumentById<Type>(
  id: string,
  database: string,
  collection: string,
) {
...
export async function createOrEditAbout(formData: FormData) {
  ...
  const editDate = formData.get("autoDate")
    ? new Date(Date.now())
    : fetchedDate;
  const id =
    formData.get("mongoId") == ""
      ? undefined
      : (formData.get("mongoId")?.toString() ?? undefined);
  ...
  try {
    ...
    await db
      ...
      .updateOne(
        { _id: new ObjectId(id) },
        { $set: { editDate, title, description, author } },
        { upsert: true },
      );
    return;
  } catch (e) {
